...
namespace DAL.BaseInfo
{
    public class EQUIPMENT_PARAM_INFO
    {
        ...
        {
            return OracleHelper.SelectedToIList<EQUIPMENT_PARAM_INFO_Entity>(PubConstant.ConnectionString, CommandType.Text,
            @"
SELECT
        EQUIPMENT_ID,
        FACTORY_ID,
...
FROM (SELECT
        ...
        FROM (  SELECT 
                    ...
                    TO_CHAR (UPDATE_DATE, 'MM/DD/YYYY HH24:MI:SS') UPDATE_DATE,
                    IS_SC_PARAM
                FROM EQUIPMENT_PARAM_INFO ) T1,          
            (  SELECT COUNT (1) TOTAL FROM EQUIPMENT_PARAM_INFO ) T3
...
WHERE T2.ROWINDEX > (:PAGE_NUMBER - 1) * :PAGE_SIZE
  ", new[]{
     ...
     OracleHelper.MakeInParam("PAGE_NUMBER",OracleType.Number,pageNumber)
        ...
        public List<EQUIPMENT_PARAM_INFO_Entity> GetDataById(string EQUIPMENT_ID, string PARAMETER_ID, string PRODUCT_TYPE_ID, string PRODUCT_PROC_TYPE_ID, string FACTORY_ID, string queryStr)
        {
            return OracleHelper.SelectedToIList<EQUIPMENT_PARAM_INFO_Entity>(PubConstant.ConnectionString, CommandType.Text, @"
SELECT
        EQUIPMENT_ID,
...
SELECT A.EQUIPMENT_ID,
  ...
  FROM EQUIPMENT_PARAM_INFO A, PARAMETER_LIST PARAM
 WHERE     A.EQUIPMENT_ID = :EQUIPMENT_ID
       ...
       AND PARAM.PRODUCT_TYPE_ID = :PRODUCT_TYPE_ID
       AND PARAM.PRODUCT_PROC_TYPE_ID = :PRODUCT_PROC_TYPE_ID
...
" + queryStr + " ORDER BY DISP_ORDER_NO", new[]{
        ...
        public bool GetDataValidateId(string EQUIPMENT_ID, string PARAMETER_ID, string PRODUCT_TYPE_ID, string PRODUCT_PROC_TYPE_ID, string FACTORY_ID)
        {
...
SELECT COUNT (1)
  FROM EQUIPMENT_PARAM_INFO
 WHERE EQUIPMENT_ID=:EQUIPMENT_ID AND PARAMETER_ID=:PARAMETER_ID AND PRODUCT_TYPE_ID=:PRODUCT_TYPE_ID AND PRODUCT_PROC_TYPE_ID=:PRODUCT_PROC_TYPE_ID AND FACTORY_ID=:FACTORY_ID
",
                new[]{
            ...
            return OracleHelper.ExecuteNonQuery(PubConstant.ConnectionString, CommandType.Text,
                @"
INSERT INTO EQUIPMENT_PARAM_INFO (
        EQUIPMENT_ID,
        FACTORY_ID,
        PRODUCT_TYPE_ID,
        PRODUCT_PROC_TYPE_ID,
...
", new[]{
            ...
            OracleHelper.MakeInParam("DISP_ORDER_IN_TASK",OracleType.Number,0,entity.DISP_ORDER_IN_TASK),
        ...
        public int PostEdit(EQUIPMENT_PARAM_INFO_Entity entity)
        {
            return OracleHelper.ExecuteNonQuery(PubConstant.ConnectionString, CommandType.Text,
                @"
UPDATE EQUIPMENT_PARAM_INFO SET 
    DISP_ORDER_NO=:DISP_ORDER_NO,
    DISP_ORDER_IN_TASK=:DISP_ORDER_IN_TASK,
...
", new[]{
        ...
        public int PostDelete(EQUIPMENT_PARAM_INFO_Entity entity)
        {
            return OracleHelper.ExecuteNonQuery(PubConstant.ConnectionString, CommandType.Text,
@"
DELETE FROM EQUIPMENT_PARAM_INFO 
WHERE
    EQUIPMENT_ID=:EQUIPMENT_ID AND 
    PARAMETER_ID=:PARAMETER_ID AND 
    PRODUCT_TYPE_ID=:PRODUCT_TYPE_ID AND 
