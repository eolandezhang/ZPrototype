...
namespace DAL.BaseInfo
{
    public class PRODUCT_TYPE_LIST
    {
        ...
        {
            return OracleHelper.SelectedToIList<PRODUCT_TYPE_LIST_Entity>(PubConstant.ConnectionString, CommandType.Text, @"
SELECT
        PRODUCT_TYPE_ID,
        FACTORY_ID,
        UPDATE_USER,
...
FROM (  SELECT ROWNUM AS ROWINDEX,
     ...
     (  SELECT COUNT (1) TOTAL FROM PRODUCT_TYPE_LIST ) B
...
SELECT COUNT (1)
  FROM PRODUCT_TYPE_LIST
 WHERE PRODUCT_TYPE_ID=:PRODUCT_TYPE_ID AND FACTORY_ID=:FACTORY_ID
",
                new[]{
...
", new[]{
            ...
            OracleHelper.MakeInParam("PRODUCT_TYPE_NAME",OracleType.VarChar,20,entity.PRODUCT_TYPE_NAME),
            OracleHelper.MakeInParam("PRODUCT_TYPE_DESC",OracleType.VarChar,25,entity.PRODUCT_TYPE_DESC),
...
", new[]{
        ...
        public int PostDelete(PRODUCT_TYPE_LIST_Entity entity)
        {
            return OracleHelper.ExecuteNonQuery(PubConstant.ConnectionString, CommandType.Text,
@"
DELETE FROM PRODUCT_TYPE_LIST WHERE PRODUCT_TYPE_ID=:PRODUCT_TYPE_ID AND FACTORY_ID=:FACTORY_ID
", new[]{
            OracleHelper.MakeInParam("PRODUCT_TYPE_ID",OracleType.VarChar,15,entity.PRODUCT_TYPE_ID),
            OracleHelper.MakeInParam("FACTORY_ID",OracleType.VarChar,5,entity.FACTORY_ID)
            });
