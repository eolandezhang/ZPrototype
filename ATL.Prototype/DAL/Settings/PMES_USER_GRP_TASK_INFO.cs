...
namespace DAL.Settings
{
    public class PMES_USER_GRP_TASK_INFO
    {
        ...
        {
            return OracleHelper.SelectedToIList<PMES_USER_GRP_TASK_INFO_Entity>(PubConstant.ConnectionString, CommandType.Text,
            @"
SELECT
        PMES_USER_GROUP_ID,
        PMES_TASK_ID,
...
FROM (SELECT
        ...
        FROM (  SELECT 
                    ...
                    TO_CHAR (UPDATE_DATE, 'MM/DD/YYYY HH24:MI:SS') UPDATE_DATE,
                    VALID_FLAG
                FROM PMES_USER_GRP_TASK_INFO ) T1,          
            (  SELECT COUNT (1) TOTAL FROM PMES_USER_GRP_TASK_INFO ) T3
...
SELECT COUNT (1)
  FROM PMES_USER_GRP_TASK_INFO
 WHERE PMES_USER_GROUP_ID=:PMES_USER_GROUP_ID AND PMES_TASK_ID=:PMES_TASK_ID AND FACTORY_ID=:FACTORY_ID
",
                new[]{
            ...
            return OracleHelper.ExecuteNonQuery(PubConstant.ConnectionString, CommandType.Text,
                @"
INSERT INTO PMES_USER_GRP_TASK_INFO (
        PMES_USER_GROUP_ID,
        PMES_TASK_ID,
        FACTORY_ID,
        UPDATE_USER,
...
", new[]{
        ...
        public int PostEdit(PMES_USER_GRP_TASK_INFO_Entity entity)
        {
            return OracleHelper.ExecuteNonQuery(PubConstant.ConnectionString, CommandType.Text,
                @"
UPDATE PMES_USER_GRP_TASK_INFO SET 
...
", new[]{
        ...
        public int PostDelete(PMES_USER_GRP_TASK_INFO_Entity entity)
        {
            return OracleHelper.ExecuteNonQuery(PubConstant.ConnectionString, CommandType.Text,
@"
DELETE FROM PMES_USER_GRP_TASK_INFO 
WHERE
    PMES_USER_GROUP_ID=:PMES_USER_GROUP_ID AND 
    PMES_TASK_ID=:PMES_TASK_ID AND 
    FACTORY_ID=:FACTORY_ID
