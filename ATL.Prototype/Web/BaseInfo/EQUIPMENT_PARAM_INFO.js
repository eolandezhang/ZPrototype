var pageSize = 20;
var editIndex_EQUIPMENT_PARAM_INFO = undefined;
var addOrEdit_EQUIPMENT_PARAM_INFO = null;
var editIndex_EQUIPMENT_PROC_INFO = undefined;
var addOrEdit_EQUIPMENT_PROC_INFO = null;
var editIndex_PARAMETER_LIST = undefined;

$(function () {
    Table_EQUIPMENT_PARAM_INFO_();    

    Dialog_EQUIPMENT_PARAM_INFO();
    InitCurrentFactoryId();
    Dialog_EQUIPMENT_PROC_INFO();
    Table_EQUIPMENT_PROC_INFO_();

    Table_PARAMETER_LIST_();
});
function Table_EQUIPMENT_PARAM_INFO_() {
    $('#Table_EQUIPMENT_PARAM_INFO').datagrid({
        title: '设备参数',
        singleSelect: true,
        width: '300',
        height: '368',
        ...
        toolbar: [{
            text: '新增',
            handler: function () {                
                addOrEdit_EQUIPMENT_PARAM_INFO = 'add';
                PARAMETER_ID();                
                $('#PARAMETER_ID').combobox('readonly', false);
                $('#DISP_ORDER_NO').val('').attr('readonly', false);
                $('#DISP_ORDER_IN_TASK').val('').attr('readonly', false);
                $('#IS_SC_PARAM').val('0');
                ...
                $('#Dialog_EQUIPMENT_PARAM_INFO').dialog('open');
            }
        }, {
            text: '修改',
            handler: function () {
                addOrEdit_EQUIPMENT_PARAM_INFO = 'edit';
                var x = $('#Table_EQUIPMENT_PARAM_INFO').datagrid('getSelected');
                if (x == null) return;
                PARAMETER_ID();               
                $('#PARAMETER_ID').combogrid('setValue', x.PARAMETER_ID).combogrid('readonly', true);
                $('#DISP_ORDER_NO').val(x.DISP_ORDER_NO);
                $('#DISP_ORDER_IN_TASK').val(x.DISP_ORDER_IN_TASK);
            ...
            text: '删除',
            handler: function () {
                Deleterow_EQUIPMENT_PARAM_INFO();
            }
        }, {
            text: '保存',
            handler: function () {
                $('#Table_EQUIPMENT_PARAM_INFO').datagrid('endEdit', editIndex_EQUIPMENT_PARAM_INFO);
                var changedRow = $('#Table_EQUIPMENT_PARAM_INFO').datagrid('getChanges');
                if (changedRow.length > 0) {
                    for (var i = 0; i < changedRow.length; i++) {
                        Edit_Cell_EQUIPMENT_PARAM_INFO(changedRow[i]);
                    }
                }
                editIndex_EQUIPMENT_PARAM_INFO = undefined;
                $('#Table_EQUIPMENT_PARAM_INFO').datagrid('clearSelections');
            }
        }, {
            text: '取消',
            handler: function () {
                $('#Table_EQUIPMENT_PARAM_INFO').datagrid('rejectChanges');
            }
        }],
        frozenColumns: [[
            { field: 'PARAMETER_ID', title: '编号', align: 'left', width: 140, styler: function (value, row, index) { return 'color:blue'; } }
        ]],
        columns: [[
            { field: 'DISP_ORDER_NO', title: '序号', align: 'left', width: 40, editor: 'text' },
            { field: 'DISP_ORDER_IN_TASK', title: 'TASK序号', align: 'left', editor: 'text' },
        ...
        onClickRow: function (rowIndex, rowData) {
            editrow_EQUIPMENT_PARAM_INFO(rowIndex);
            Init_Table_PARAMETER_LIST_();
        },
        onDblClickRow: function (rowIndex, rowData) { },
        rowStyler: function (index, row) { },
        onBeforeEdit: function (index, row) {
            editIndex_EQUIPMENT_PARAM_INFO = index;
            row.editing = true;
            $('#Table_EQUIPMENT_PARAM_INFO').datagrid('refreshRow', index);
        },
...
function Init_Table_EQUIPMENT_PARAM_INFO_() {
    var x = $('#Table_EQUIPMENT_PROC_INFO').datagrid('getSelected');
    if (x == null) return;
    var y = $('#PRODUCT_TYPE_ID_SEARCH').combobox('getValue');
    if (y == '') {
        return;
    }
    var z = $('#PRODUCT_PROC_TYPE_ID_SEARCH').combobox('getValue');
    if (z == '') {
        return;
    }
    var u = $('#FACTORY_ID_SEARCH').combobox('getValue');
    if (u == '') {
        return;
    }
    var j = {
    ...
    $.ajax({
        type: 'get',
        url: '/api/EQUIPMENT_PARAM_INFO/GetDataByEquipmentId',
        data: j,
        dataType: 'json',
        success: function (data) {
            $('#Table_EQUIPMENT_PARAM_INFO').datagrid("loadData", data).datagrid('acceptChanges');
        }
    });
}

function editrow_EQUIPMENT_PARAM_INFO(index) {
    if (editIndex_EQUIPMENT_PARAM_INFO != undefined)
        $('#Table_EQUIPMENT_PARAM_INFO').datagrid('endEdit', editIndex_EQUIPMENT_PARAM_INFO);
    $('#Table_EQUIPMENT_PARAM_INFO').datagrid('beginEdit', index).datagrid('selectRow', index);
}
function Deleterow_EQUIPMENT_PARAM_INFO() {
    var row = $('#Table_EQUIPMENT_PARAM_INFO').datagrid('getSelected');
    if (row == null) {
        $.messager.show({
            title: '消息',
            msg: '请选择',
    ...
    $.messager.confirm('确认', '是否真的删除?', function (r) {
        if (r) {
            Delete_EQUIPMENT_PARAM_INFO(row);
            $('#Table_EQUIPMENT_PARAM_INFO').datagrid('deleteRow', $('#Table_EQUIPMENT_PARAM_INFO').datagrid('getRowIndex'));
        }
    });
}

function Dialog_EQUIPMENT_PARAM_INFO() {
    $('#Dialog_EQUIPMENT_PARAM_INFO').dialog({
        toolbar: [{
            text: '保存',
            iconCls: 'icon-save',
            handler: function () {
                if (addOrEdit_EQUIPMENT_PARAM_INFO == 'add') {
                    Add_EQUIPMENT_PARAM_INFO();
                }
                else if (addOrEdit_EQUIPMENT_PARAM_INFO == 'edit') {
                    Edit_EQUIPMENT_PARAM_INFO();
                }
            }
        }]
    });
}
function Add_EQUIPMENT_PARAM_INFO() {
    var x = $('#Table_EQUIPMENT_PROC_INFO').datagrid('getSelected');
    if (x == null) return;
    ...
    if (z == '') return;
    var u = $('#PRODUCT_PROC_TYPE_ID_SEARCH').combobox('getValue');
    if (u == '') return;
    var v = $('#PARAMETER_ID').combobox('getValue');
    if (v == '') return;
    var j = {
        'EQUIPMENT_ID': x.EQUIPMENT_ID,
        'FACTORY_ID': y,
        'PRODUCT_TYPE_ID': z,
        'PRODUCT_PROC_TYPE_ID': u,
        'PARAMETER_ID': v,
        'DISP_ORDER_NO': $('#DISP_ORDER_NO').val(),
        'DISP_ORDER_IN_TASK': $('#DISP_ORDER_IN_TASK').val(),
    ...
    $.ajax({
        type: 'post',
        url: '/api/EQUIPMENT_PARAM_INFO/PostAdd',
        ...
        success: function (data) {
            if (data > 0) {
                ...
                Init_Table_EQUIPMENT_PARAM_INFO_();
                $('#Dialog_EQUIPMENT_PARAM_INFO').dialog('close');
            } else {
                $.messager.show({
                    title: '消息',
                    msg: '失败',
...
function Edit_EQUIPMENT_PARAM_INFO() {
    var x = $('#Table_EQUIPMENT_PARAM_INFO').datagrid('getSelected');
    if (x == null) return;
    var j = {
        'EQUIPMENT_ID': x.EQUIPMENT_ID,
    ...
    $.ajax({
        type: 'post',
        url: '/api/EQUIPMENT_PARAM_INFO/PostEdit',
        data: JSON.stringify(j),
        dataType: 'json',
        contentType: 'application/json',
        success: function (data) {
...
function Edit_Cell_EQUIPMENT_PARAM_INFO(row) {
    var j = {
        ...
        'DISP_ORDER_IN_TASK': row.DISP_ORDER_IN_TASK,
...
function Delete_EQUIPMENT_PARAM_INFO(row) {
    var j = {
        'EQUIPMENT_ID': row.EQUIPMENT_ID,
        'PARAMETER_ID': row.PARAMETER_ID,
        'PRODUCT_TYPE_ID': row.PRODUCT_TYPE_ID,
    ...
    $.ajax({
        type: 'post',
        url: '/api/EQUIPMENT_PARAM_INFO/PostDelete',
        data: JSON.stringify(j),
        dataType: 'json',
        contentType: 'application/json',
        success: function (data) {
...
function Table_PROCESS_LIST(data) {
    $('#Table_PROCESS_LIST').datagrid({
        ...
        onClickRow: function (rowIndex, rowData) {
            Init_Table_EQUIPMENT_PROC_INFO_();
        }
    });
...
//工序-设备编号
function Table_EQUIPMENT_PROC_INFO_() {
    $('#Table_EQUIPMENT_PROC_INFO').datagrid({
        title: '工序-设备编号',
        singleSelect: true,
        width: '240',
        height: '368',
        ...
        toolbar: [{
            text: '新增',
            handler: function () {
                addOrEdit_EQUIPMENT_PROC_INFO = 'add';
                EQUIPMENT_ID();
                $('#EQUIPMENT_ID').combobox('readonly', false);
                $('#UPDATE_USER_EQUIPMENT_PROC_INFO').val('').attr('readonly', true);
                $('#UPDATE_DATE_EQUIPMENT_PROC_INFO').val('').attr('readonly', true);
                $('#REMARK').val('').attr('readonly', false);
                $('#Dialog_EQUIPMENT_PROC_INFO').dialog('open');
            }
        }, {
            text: '修改',
            handler: function () {
                addOrEdit_EQUIPMENT_PROC_INFO = 'edit';
                var x = $('#Table_EQUIPMENT_PROC_INFO').datagrid('getSelected');
                if (x == null) return;
                EQUIPMENT_ID();
                $('#EQUIPMENT_ID').combobox('setValue', x.EQUIPMENT_ID).combobox('readonly', true);
                $('#UPDATE_USER_EQUIPMENT_PROC_INFO').val(x.UPDATE_USER).attr('readonly', true);
                $('#UPDATE_DATE_EQUIPMENT_PROC_INFO').val(x.UPDATE_DATE).attr('readonly', true);
                $('#REMARK').val(x.REMARK);
                $('#Dialog_EQUIPMENT_PROC_INFO').dialog('open');
            }
        }, {
            text: '删除',
            handler: function () {
                Deleterow_EQUIPMENT_PROC_INFO();
            }
        }, {
            text: '保存',
            handler: function () {
                $('#Table_EQUIPMENT_PROC_INFO').datagrid('endEdit', editIndex_EQUIPMENT_PROC_INFO);
                var changedRow = $('#Table_EQUIPMENT_PROC_INFO').datagrid('getChanges');
                if (changedRow.length > 0) {
                    for (var i = 0; i < changedRow.length; i++) {
                        Edit_Cell_EQUIPMENT_PROC_INFO(changedRow[i]);
                    }
                }
                editIndex_EQUIPMENT_PROC_INFO = undefined;
                $('#Table_EQUIPMENT_PROC_INFO').datagrid('clearSelections');
            }
        }, {
            text: '取消',
            handler: function () {
                $('#Table_EQUIPMENT_PROC_INFO').datagrid('rejectChanges');
            }
        }],
        frozenColumns: [[
            { field: 'EQUIPMENT_ID', title: '编号', align: 'left', width: 140, styler: function (value, row, index) { return 'color:blue'; } }
        ]],
        columns: [[
        ...
        onClickRow: function (rowIndex, rowData) {
            editrow_EQUIPMENT_PROC_INFO(rowIndex);
            Init_Table_EQUIPMENT_PARAM_INFO_();
        },
        onDblClickRow: function (rowIndex, rowData) { },
        rowStyler: function (index, row) { },
        onBeforeEdit: function (index, row) {
            editIndex_EQUIPMENT_PROC_INFO = index;
            row.editing = true;
            $('#Table_EQUIPMENT_PROC_INFO').datagrid('refreshRow', index);
        },
        onAfterEdit: function (index, row) {
            row.editing = false;
            $('#Table_EQUIPMENT_PROC_INFO').datagrid('refreshRow', index);
...
function Init_Table_EQUIPMENT_PROC_INFO_() {
    ...
    var u = $('#Table_EQUIPMENT_TYPE_LIST').datagrid('getSelected');
    if (u == null) return;
    ...
    $.ajax({
        type: 'get',
        url: '/api/EQUIPMENT_PROC_INFO/GetDataByProcessIdAndTypeId',
        data: j,
        dataType: 'json',
        success: function (data) {
            $('#Table_EQUIPMENT_PROC_INFO').datagrid("loadData", data).datagrid('acceptChanges');
        }
    });
}

function editrow_EQUIPMENT_PROC_INFO(index) {
    if (editIndex_EQUIPMENT_PROC_INFO != undefined)
        $('#Table_EQUIPMENT_PROC_INFO').datagrid('endEdit', editIndex_EQUIPMENT_PROC_INFO);
    $('#Table_EQUIPMENT_PROC_INFO').datagrid('beginEdit', index).datagrid('selectRow', index);
}
function Deleterow_EQUIPMENT_PROC_INFO() {
    var row = $('#Table_EQUIPMENT_PROC_INFO').datagrid('getSelected');
    if (row == null) {
        $.messager.show({
            title: '消息',
            msg: '请选择',
    ...
    $.messager.confirm('确认', '是否真的删除?', function (r) {
        if (r) {
            Delete_EQUIPMENT_PROC_INFO(row);
        }
    });
}
function Dialog_EQUIPMENT_PROC_INFO() {
    $('#Dialog_EQUIPMENT_PROC_INFO').dialog({
        toolbar: [{
            text: '保存',
            iconCls: 'icon-save',
            handler: function () {
                if (addOrEdit_EQUIPMENT_PROC_INFO == 'add') {
                    Add_EQUIPMENT_PROC_INFO();
                }
                else if (addOrEdit_EQUIPMENT_PROC_INFO == 'edit') {
                    Edit_EQUIPMENT_PROC_INFO();
                }
            }
        }]
    });
    $('#alert_EQUIPMENT_ID').hide();
}
function Validate_EQUIPMENT_PROC_INFO() {
    if (!(
...
function Add_EQUIPMENT_PROC_INFO() {
    var x = $('#Table_PROCESS_LIST').datagrid('getSelected');
    if (x == null) return;
    var y = $('#EQUIPMENT_ID').combobox('getValue');
    if (y == '') {
        return;
    }
    var z = $('#FACTORY_ID_SEARCH').combobox('getValue');
    ...
    var j = {
        ...
        'EQUIPMENT_ID': y,
        'FACTORY_ID': z,
        'UPDATE_USER': $('#UPDATE_USER_EQUIPMENT_PROC_INFO').val(),
        'EQUIPMENT_TYPE_ID': u.EQUIPMENT_TYPE_ID,
        'UPDATE_DATE': $('#UPDATE_DATE_EQUIPMENT_PROC_INFO').val(),
        'REMARK': $('#REMARK').val()
    };
    $.ajax({
        type: 'post',
        url: '/api/EQUIPMENT_PROC_INFO/PostAdd',
        data: JSON.stringify(j),
        dataType: 'json',
        contentType: 'application/json',
        success: function (data) {
            if (data > 0) {
                ...
                Init_Table_EQUIPMENT_PROC_INFO_();
                $('#Dialog_EQUIPMENT_PROC_INFO').dialog('close');
            } else {
                $.messager.show({
                    title: '消息',
                    msg: '失败',
...
function Edit_EQUIPMENT_PROC_INFO() {
    var x = $('#Table_EQUIPMENT_PROC_INFO').datagrid('getSelected');
    if (x == null) return;
    ...
    $.ajax({
        type: 'post',
        url: '/api/EQUIPMENT_PROC_INFO/PostEdit',
        data: JSON.stringify(j),
        dataType: 'json',
        contentType: 'application/json',
        success: function (data) {
...
function Edit_Cell_EQUIPMENT_PROC_INFO(row) {
    var j = {
...
function Delete_EQUIPMENT_PROC_INFO(row) {
    var j = {
        'PROCESS_ID': row.PROCESS_ID,
        'EQUIPMENT_ID': row.EQUIPMENT_ID,
        'FACTORY_ID': row.FACTORY_ID
    ...
    $.ajax({
        type: 'post',
        url: '/api/EQUIPMENT_PROC_INFO/PostDelete',
        data: JSON.stringify(j),
        dataType: 'json',
        contentType: 'application/json',
        success: function (data) {
...
function EQUIPMENT_ID() {
    ...
    $('#EQUIPMENT_ID').combobox({
        ...
        textField: 'EQUIPMENT_ID',
        url: '/api/EQUIPMENT_LIST/GetDataByFactoryId?FACTORY_ID=' + z + "&queryStr= AND VALID_FLAG='1'",
        method: 'get',
        panelHeight: 100,
        editable: true,
        onSelect: function (record) {
        ...
        onChange: function (newValue, oldValue) {
            $('#EQUIPMENT_ID').combobox('setValue', newValue.toUpperCase());
            if (newValue != null) {
                var x = $('#EQUIPMENT_ID').combobox('getData');
                var flag = false;
                $.each(x, function (i) {
                    if (x[i].EQUIPMENT_ID.toUpperCase() == newValue.toUpperCase()) {
                        $('#EQUIPMENT_ID').combobox('setValue', x[i].EQUIPMENT_ID);
                        flag = true;
                        return;
                    }
                });
...
function PARAMETER_ID() {
    ...
    $('#PARAMETER_ID').combogrid({
        url: "/api/PARAMETER_LIST/GetDataByPType?FACTORY_ID=" + factoryId + "&PRODUCT_TYPE_ID=" + productTypeId + "&PRODUCT_PROC_TYPE_ID=" + produceProcTypeId + "&PARAM_TYPE_ID=" + PARAM_TYPE_ID + "&queryStr=",
